...
int* random_array(int begin, int end, int array_length){
    if(begin > end) swap(&begin, &end, sizeof(int));
...
void print_if_array(int* v, int n, bool(*condicao)(int)){
    printf("( ");
    for(int i = 0; i<n && condicao(v[i]); i++) printf("%d, ", v[i]);
    printf(")");
}
...
int find_array(int* v, int n, int e){
    for(int i=0; i<n; i++) if(v[i] == e) return i;
    return -1;
}
...
void printz_array(int* v, int n){
    printf("( ");
    for(int i = 0; i<n && v[i]; i++) printf("%d, ", v[i]) ;
    printf(")");
}
...
int min_array(int* v, int n){
    int min = 0;
    for(int i = 1; i < n; i++) if(v[min] > v[i]) min = i;
    return min;
}

int mult_array(int* v, int n){
    int m = 1;
    for(int i = 0; i < n ; i++) m *= v[i] ? v[i] : 1;
    return m;
}
