...
unsigned char intervalo_pedras = 3;
...
{
    // Verificando colisão com pedra
    if(matriz_pedras[tiro.x][tiro.y])
    {   
        tiro.y = 15;
        matriz_pedras[tiro.x][tiro.y] = -1;
    }

    if(matriz_pedras[tiro.x][tiro.y-1])
    {
        matriz_pedras[tiro.x][tiro.y-1] = -1;
        tiro.y = 15;
    }
}
...
void gerar_pedras()
{
    ...
    if(pedras_time % intervalo_pedras == 0)
    {
        matriz_pedras[random_lancer_pedras][15] = 1;
        random_lancer_pedras++; // TODO: Tirar intercalação...
    }
    pedras_time++;
    pedras_time %= intervalo_pedras;
}
...
{
    if(pedras_time % intervalo_pedras == 2)
        for(char i = 0; i < 2; i++)
            for(char j = 0; j < 16; j++)
                mostrar_pedra(i, j);
}
