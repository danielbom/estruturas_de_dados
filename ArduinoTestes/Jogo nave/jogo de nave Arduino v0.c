...
{	
    ...
    for(k=0;k<7;k++)
      	cmd_LCD(pgm_read_byte(&carac3[k]),1);	//grava 8 bytes na DDRAM começando no end. 0x50 
   	cmd_LCD(0x00,1);				//apaga última posição do end. da CGRAM para evitar algum dado espúrio
  
  	cmd_LCD(0x58,0);				//endereço base para gravar novo segmento  0x58
    for(k=0;k<7;k++)
      	cmd_LCD(pgm_read_byte(&carac4[k]),1);	//grava 8 bytes na DDRAM começando no end. 0x58 
   	cmd_LCD(0x00,1);				//apaga última posição do end. da CGRAM para evitar algum dado espúrio
  
  	cmd_LCD(0x60,0);				//endereço base para gravar novo segmento  0x60
    for(k=0;k<7;k++)
      	cmd_LCD(pgm_read_byte(&carac5[k]),1);	//grava 8 bytes na DDRAM começando no end. 0x60 
   	cmd_LCD(0x00,1);				//apaga última posição do end. da CGRAM para evitar algum dado espúrio	
  
  	cmd_LCD(0x68,0);				//endereço base para gravar novo segmento  0x68
    for(k=0;k<7;k++)
      	cmd_LCD(pgm_read_byte(&carac6[k]),1);	//grava 8 bytes na DDRAM começando no end. 0x68 
   	cmd_LCD(0x00,1);				//apaga última posição do end. da CGRAM para evitar algum dado espúrio
  	   
    for(;;){	//laço infinito
      if(valor==0x05)		//se o valor for igual a 0xF, zera o valor, 
				valor=0;
			else 				//se não o incrementa
				valor++;
      cmd_LCD(0x80,0);				//endereça a posição para escrita dos caracteres
   	  cmd_LCD(valor,1);				//apresenta primeiro caractere 0x00
      cmd_LCD(0x14,0);				//desloca o cursor para a direita	
      _delay_ms(500);		//atraso para incremento automático do nr. no display
    }
